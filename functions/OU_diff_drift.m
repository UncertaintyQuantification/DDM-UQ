function pos = OU_diff_drift(len,N, pos0,drift_dir,step,rho,mu)
% skip a step ever other step
    ...
    if drift_dir==1
        ...
        for i=2:len
            pos(:,i,1) = rho.*(pos(:,i-1,1)-pos0(:,1)-(i-2)*mu*cos(theta))+pos0(:,1)+(i-1)*mu*cos(theta)+step*sqrt((1-rho^2)).*randn(N,1);
            pos(:,i,2) = rho.*(pos(:,i-1,2)-pos0(:,2)-(i-2)*mu*sin(theta))+pos0(:,2)+(i-1)*mu*sin(theta)+step*sqrt((1-rho^2)).*randn(N,1);
...
%             poscurr(:,i,2) = rho.*(poscurr(:,i-1,2)-pos1_y-(i-2)*mu.*sin(theta))+pos1_y+(i-1)*mu.*sin(theta)+step*sqrt((1-rho^2)).*randn(N,1);
        ...
        for i=2:len
            pos(:,i,1) = rho.*(pos(:,i-1,1)-pos0(:,1)-(i-2)*mu)+pos0(:,1)+(i-1)*mu+step*sqrt((1-rho^2)).*randn(N,1);
            pos(:,i,2) = rho.*(pos(:,i-1,2)-pos0(:,2)-(i-2)*mu)+pos0(:,2)+(i-1)*mu+step*sqrt((1-rho^2)).*randn(N,1);
