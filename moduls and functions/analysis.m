...
%                     area, unaggregated MSD for different qs and lag  
%                     times, and aggregate MSD vs lag time with error bar 
%                     that stands for 95% predictive interval. A line of
%                     true MSD for comparsion will be added if the
%                     reference value is provided. (For simulated data
%                     the generated using simulation module, median of 
%                     numerical MSD will be considered as true MSD). 
%% Input
...
%   t_Cut- final length of dt that we can get. 
...
n_sample = 300;
lower95_Dqt_sample = NaN(length(q_selected_index),n_time);
upper95_Dqt_sample = NaN(length(q_selected_index),n_time);
lower95_fqt_sample = NaN(length(q_selected_index),n_time);
upper95_fqt_sample = NaN(length(q_selected_index),n_time);
for i = 1:n_q_max 
...
subplot(2,2,1) %plotting MSD vs delta t for each q 
min_pred_ddm = min(min(model.pred_Dqt(10:10:60,:)));
max_pred_ddm = max(max(model.pred_Dqt(10:10:60,:)));
for i = 10:10:60
    loglog(params.dt,model.pred_Dqt(i,:),'-','Display',strcat('i = ', num2str(i)))
    hold on
    legendInfo{i/10} = ['q = ' num2str(round(q(i),2))]; 
    hold on
end
lgd=legend(legendInfo,'AutoUpdate','off','Location','northwest','FontSize',10);
lgd.NumColumns = 2;
for i = 10:10:60
    loglog(params.dt(model.index_dt_selected), sub_ddm(i,:), '.k', 'MarkerFaceColor','k','MarkerSize', markSize)
    hold on
end
for i = 10:10:60
    a = patch([params.dt fliplr(params.dt)], [lower95_Dqt_sample(i,:) fliplr(upper95_Dqt_sample(i,:))], [0.8 0.8 0.8],'LineStyle','none');
    uistack(a,'bottom')
    hold on
end
...
xlabel('\Delta t')
ylabel('D(q,\Deltat)')
xlim([params.dt(1)-0.2*params.dt(1) params.dt(end)+0.3*params.dt(end)])
ylim([min_pred_ddm-0.2*min_pred_ddm max_pred_ddm+0.5*max_pred_ddm])
title('Predictive image structure function')
    
subplot(2,2,2)
min_pred_fqt = min(min(model.fqt(10:10:60,:)));
max_pred_fqt = max(max(model.fqt(10:10:60,:)));
for i = 10:10:60
    semilogx(params.dt,model.fqt(i,:),'-','Display',strcat('i = ', num2str(i)))
    hold on
    legendInfo{i/10} = ['q = ' num2str(round(q(i),2))]; 
    hold on
end
...
lgd.NumColumns = 2;
for i = 10:10:60
    a = patch([params.dt fliplr(params.dt)], [lower95_fqt_sample(i,:) fliplr(upper95_fqt_sample(i,:))], [0.8 0.8 0.8],'LineStyle','none');
    uistack(a,'bottom')
    hold on
end
set(gca, 'XScale', 'log')
...
for i = 1:num_testing1
    loglog(params.dt,median_MSD_record(i,:),'-','Display',strcat('i = ', num2str(i)))
    hold on
end
xlabel('\Delta t')
ylabel('median MSD(q,\Deltat)')
xlim([params.dt(1)-0.2*params.dt(1) params.dt(model.t_Cut)+0.3*params.dt(model.t_Cut)])
...
if(isfield(model,'MSD_truth'))
    ...
    errorbar(params.dt(1:model.t_Cut),model.pred_MSD_sample, yneg, ypos,'linestyle','none','Marker', '.','MarkerSize',11)
    xlabel('\Delta t')
    ylabel('MSD')
    set(gca,'xscale','log');
    set(gca,'yscale','log');
    hold on 
    loglog(params.dt(1:model.t_Cut),model.MSD_truth(1:model.t_Cut));
    xlim([params.dt(1)-0.2*params.dt(1) params.dt(model.t_Cut)+0.2*params.dt(model.t_Cut)])
    ...
    set(gca,'yscale','log');
    xlim([params.dt(1)-0.2*params.dt(1) params.dt(model.t_Cut)+0.2*params.dt(model.t_Cut)])
    % ylim([min(model.lower95_MSD_sample)-0.2*abs(max(model.upper95_MSD_sample)) max(model.upper95_MSD_sample)+0.2*abs(max(model.upper95_MSD_sample))])
    title('Mean squared displacement')
end
toc
end