...
%	noiselevel- noise level, default value is 0.2.
%	sigma- width of the Gaussian, default value is 2.
%	Imax- maximum intensity, default value is 255.              
% 	Imin- minimum intensity at the center of the particle, default value 
%         is 0.
%	I0- background intensity default value is 20.
%% Output 
...
%% Examples (To run the example, copy the code to example.m file.)
...
if(~isfield(params,'noiselevel'))
    ...
    model.noiselevel = params.noiselevel;
end
...
if(~isfield(params,'Imin'))
    model.Imin = 0;
else
    model.Imin = params.Imin;
end
...
if(~isnumeric(model.noiselevel))
    error('noise level should be a numerical value');  
end
...
if(~isnumeric(model.Imax))
    error('maximum intensity should be a numerical value');  
end

if(~isnumeric(model.Imin))
    error('minimum intensity at the center of the particle should be a numerical value');  
end

if(~isnumeric(model.I0))
    error('minimum intensity for the background should be a numerical value');  
end
...
% numerical MSD
msdi = nan(model.N,model.len-1); %dim=50*999, MSD for each particle 
...
msd = nanmean(msdi,1); %Do we need to conside particle that out of frame?
...
if model.save==1
    ...
    fprintf(fileID,'maximum intensity = %d \n', model.Imax);
    fprintf(fileID,'image noise level = %d \n', model.I0);
    fclose(fileID);
    pos = model.pos;
    I = model.I;
